...
//			+ "AND a.delete_flg = false";
	...
	public final String GET_ALL_APPLICANTS_BY_STATUS = "SELECT "
			...
			+ "            WHEN   "
			+ "                (SELECT COUNT(*)   "
			+ "                 FROM t_rejected_applicant ra   "
			+ "                 WHERE ra.applicant_id_pk = a.id_pk) > 0  "
			+ "            OR   "
			+ "                (SELECT COUNT(*)   "
			+ "                 FROM t_evaluated_applicant eaa   "
			+ "                 LEFT JOIN t_evaluation_details edd   "
			+ "                 ON edd.evaluated_applicant_id_pk = eaa.id_pk   "
			+ "                 WHERE eaa.applicant_id_pk = a.id_pk   "
			+ "                 AND edd.total < 60) > 0  "
			+ "            THEN TRUE  "
			+ "            ELSE FALSE  "
			+ "        END  "
			+ "    ) AS has_resubmitted_or_low_evaluation,"
			+ "(  "
			+ "        CASE   "
			+ "            WHEN a.status > 0 THEN (  "
			+ "                SELECT DISTINCT   "
			+ "                    CONCAT(u2.first_name, ' ', u2.last_name) AS full_name  "
			+ "                FROM   "
			+ "                    t_rejected_applicant r2  "
			+ "                JOIN   "
			+ "                    m_user_information u2   "
			+ "                    ON u2.id_pk = r2.created_by   "
			+ "                    AND u2.delete_flg = false  "
			+ "                WHERE   "
			+ "                    r2.applicant_id_pk = a.id_pk  "
			+ "                    AND r2.delete_flg = false  "
			+ "            )  "
			+ "            ELSE ''  "
			+ "        END  "
